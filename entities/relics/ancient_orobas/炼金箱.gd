...
func on_picked_up(run_stats: RunStats, char_stats: CharacterStats, _select_deck_view: DeckView) -> void:
	...
	var candidates = ItemPool.get_potions(Potion.COLOR.COLORLESS + char_stats.color, ItemPool.potion_rarity_mask)
	candidates.shuffle()
	var random_potions = candidates.slice(0, run_stats.max_potion_slots)
